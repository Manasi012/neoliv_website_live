...
const IntroVideo = () => {
  ...
  if (location.pathname === "/" && !hasPlayed) {
    if (mobile) {
      const checkOrientation = () => {
        setIsPortrait(window.innerHeight > window.innerWidth);
      };
...
<button
  ...
  className="absolute bg-white bg-opacity-80 text-black md:px-5 md:py-2 px-14 py-2 rounded-full text-sm font-medium shadow-md hover:bg-opacity-100 transition-all duration-300 z-50"
  style={{
    top: "50%",
    left: "50%",
